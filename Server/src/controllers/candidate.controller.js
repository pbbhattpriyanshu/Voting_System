...
export const registerCandidate = async (req, res) => {
  ...
  try {
    ...
    const newCandidate = new Candidate({name, age, party, symbolPic, isApproved});
    await newCandidate.save();

    console.log("Candidate is Created Sucessfully");
    return res.status(201).json({ success: true, message: "Candidate registered successfully", candidate: newCandidate });
